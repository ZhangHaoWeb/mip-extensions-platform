...
define(function (require) {
    ...
    function pushResult(src) {
        ...
        if (self.preLoad) {
            self.pn = parseInt(self.pn) + 1;
        } else {
            // self.pn = 0;
            self.preLoad = 'preLoad';
        }

        var url = getUrl(src, self.pn, self.rn, self.normal);
        fetchJsonp(url, {
            ...
            if (!data.status && data.data) {
                ...
                self.normal = data.data.normal;
                if (!data.data.hasMore) {
                    self.button.innerHTML = '';
                    self.button.className = 'mip-list-more';
     ...
     * @param  {integer}  normal    server分页用
     * @return {string}         拼接好的 url
     */
    function getUrl(src, pn, rn, normal) {
        if (!src) {
    ...
    customElement.prototype.firstInviewCallback = function () {
        ...
        self.normal = element.getAttribute('normal') || 0;
        ...
        if (element.hasAttribute('preLoad')) {
            url = getUrl(src, self.pn, self.rn, self.normal);
            fetchJsonp(url, {
                jsonpCallback: 'callback',
                timeout: self.timeout
            }).then(function (res) {
